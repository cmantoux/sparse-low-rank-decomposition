...
# We choose the parameters that best reconstruct the
# deviations V from the template network T

print("Selecting the best parameters for the reconstruction.")

# Define optimization space
SPACE = [
    skopt.space.Real(0, 50, name='lambd', prior='uniform'),
    skopt.space.Real(0, 5, name='rho', prior='uniform'),
    skopt.space.Real(0, 50, name='mu', prior='uniform'),
    skopt.space.Real(0, 5, name='nu', prior='uniform'),
]
...
def objective(**params):
    ...
    # Perform optimization for the L1 model with Douglas-Rachford
    m = dr.SparseLowRank(**all_params)
    m.fit(samples)

    # Compute the relative RMSE for V
    result = relative_error(m.V, V_init)
...
results = skopt.forest_minimize(objective, SPACE, n_calls=100, random_state=0)
...
print("\nSelected parameters:")
print("lambda = ", results['x'][0])
print("rho    = ", results['x'][1])
print("mu     = ", results['x'][2])
print("nu     = ", results['x'][3])

# Train model with selected parameters
model = dr.SparseLowRank(*results['x'], theta=theta_dr, tau=tau_dr, niter=niter, progress=False)
model.fit(samples)
